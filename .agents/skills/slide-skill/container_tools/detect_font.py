...
def main() -> None:
    ...
    if args.include_missing:
        ...
        payload["font_missing_by_slide"] = font_missing_by_slide
    if args.include_substituted:
        payload["font_substituted_overall"] = sorted(font_substituted_overall)
        payload["font_substituted_by_slide"] = font_substituted_by_slide
    ...
    if args.include_missing:
        ...
        for family in sorted(font_missing_overall):
            print(f"- {family}")
        print("font_missing_by_slide:")
        if font_missing_by_slide:
            for slide_num in sorted(font_missing_by_slide):
                print(f"- slide {slide_num}: {', '.join(font_missing_by_slide[slide_num])}")
        else:
            print("- none")

    if args.include_substituted:
        print("font_substituted_overall:")
        for family in sorted(font_substituted_overall):
            print(f"- {family}")
        print("font_substituted_by_slide:")
        if font_substituted_by_slide:
            for slide_num in sorted(font_substituted_by_slide):
                print(f"- slide {slide_num}: {', '.join(font_substituted_by_slide[slide_num])}")
        else:
            print("- none")
